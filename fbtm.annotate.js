fibity.manager.app = angular.module('fbtmApp', [
  'ngRoute',
  'chartjs',
  'ngAnimate',
  'ui.calendar',
...
fibity.manager.api.init = function (apiRoot) {
  ...
  gapi.client.load('manager', 'v1beta1', callback, apiRoot);  //gapi.client.load('dev', 'v1beta1', callback, apiRoot);
...
'use strict';
(function () {
  var chartjs = angular.module('chartjs', []), chartTypes = {
      line: 'Line',
      bar: 'Bar',
      radar: 'Radar',
      polar: 'PolarArea',
      ...
      var w = element.parent().prop('offsetWidth'), h = element.parent().prop('offsetHeight');
  ...
  chartjs.factory('chartFactory', function () {
    return function (chartType) {
      var chartType = chartTypes[chartType], extractSpecOpts = function (opts, attrs) {
          var i = opts.length, extracted = {}, cv;
          while (i--) {
            cv = attrs[opts[i]];
      ...
      return {
        ...
        link: function postLink(scope, element, attrs) {
          var ctx = element[0].getContext('2d'), chart = new Chart(ctx), chartOpts = {}, specOpts = [], autofit = scope.autofit, bound = false, drawChart = function (value, forceRedraw) {
    ...
    return {
      ...
      link: function (originalScope, element, attrs, ngModel) {
        var scope = originalScope.$new(),
          ...
          timeFormat, appendToBody = angular.isDefined(attrs.syTimepickerAppendToBody) ? originalScope.$eval(attrs.syTimepickerAppendToBody) : syTimepickerPopupConfig.appendToBody;
        ...
        var syTimepickerEl = angular.element(popupEl.children()[0]), syTimepickerOptions = {};
...
]), angular.module('dialogs', ['dialogs.services']).run([
  '$templateCache',
  function (o) {
              ...
              if (!angular.equals($scope.chart.formatters[formatType], $scope.oldChartFormatters[formatType])) {
                ...
                if (formatType === 'color') {
                  ...
                  for (var i = 0; i < $scope.chart.formatters[formatType].length; i++) {
                    $scope.formatters[formatType].push(new formatClass($scope.chart.formatters[formatType][i]));
                  }
                }
              }
...
EventValidator.prototype = {
  initialize: function (eventList, event) {
    //config
    /*List*/
  ...
  isValid: function (eventList, event) {
    var result = true;
    //algoritmo
  ...
  testCase1: function (index) {
    var result = false;
    if (this.event.start < this.eventList[index].start && this.event.end > this.eventList[index].start && this.event.end <= this.eventList[index].end)
      result = true;
    return result;
  },
  testCase2: function (index) {
    var result = false;
    if (this.event.start > this.eventList[index].start && this.event.start < this.eventList[index].end && this.event.end >= this.eventList[index].end)
      result = true;
    return result;
  },
  testCase3: function (index) {
    var result = false;
    if (this.event.start >= this.eventList[index].start && this.event.start < this.eventList[index].end && this.event.end > this.eventList[index].start && this.event.end <= this.eventList[index].end)
      result = true;
    return result;
  },
  testCase4: function (index) {
    var result = false;
    if (this.event.start <= this.eventList[index].start && this.event.end >= this.eventList[index].end)
      result = true;
    return result;
  }
};
//configuración de las rutas
fibity.manager.app.config([
  '$routeProvider',
  function ($routeProvider) {
    ...
    }).when('/antennas', { redirectTo: '/antennas/collection' }).when('/antennas/list', {
    ...
    }).when('/infocard', { redirectTo: '/infocard/collection' }).when('/infocard/collection', {
      templateUrl: 'views/infocardcollection.html',
      controller: 'GTInfoCardsVC'
    }).when('/infocard/list', {
      templateUrl: 'views/infocardlist.html',
      controller: 'GTInfoCardsVC'
    }).when('/campaigns', { redirectTo: '/campaigns/collection' }).when('/campaigns/collection', {
      templateUrl: 'views/campaignscollection.html',
      controller: 'GTCampaignsVC'
    }).when('/campaigns/list', {
      templateUrl: 'views/campaignslist.html',
      controller: 'GTCampaignsVC'
    }).when('/customers', { redirectTo: '/customers/collection' }).when('/customers/collection', {
      templateUrl: 'views/customerscollectionview.html',
      controller: 'GTCustomersVC'
    }).when('/customers/list', { templateUrl: 'views/customerslistview.html' }).when('/schedule', { templateUrl: 'views/schedule.html' }).otherwise({ redirectTo: '/organization' });
  }
...
ASMEntity.prototype = {
  ...
  initWithId: function (kind, id, data) {
    this.config[this.kindKey] = kind;
  ...
  initFromEntity: function (entity) {
    this.initFromCache(entity.json());
  },
  initFromServer: function (kind, id, json) {
  ...
  setStringForKey: function (value, key) {
    this._data[key] = value;
  },
  intForKey: function (key) {
    return this._data[key];
  },
  setIntForKey: function (value, key) {
    this._data[key] = value;
  },
  doubleForKey: function (key) {
    return this._data[key];
  },
  setDoubleForKey: function (value, key) {
    this._data[key] = value;
  },
  boolForKey: function (key) {
    return this._data[key] == true || this._data[key] == false ? this._data[key] : undefined;
  },
  setBoolForKey: function (value, key) {
    if (value == true || value == false)
      this._data[key] = value;
  },
  listForKey: function (key) {
    return this._data[key];
  },
  setListForKey: function (value, key) {
    this._data[key] = value;
  },
  addItemToListForKey: function (item, key) {
    if (this._data[key] == null)
      this._data[key] = [];
    this._data[key].push(item);
  },
  removeIndexInListForKey: function (index, key) {
    //DART: this._data[key].removeAt(index);
    index > -1 ? this._data[key].splice(index, 1) : null;
  },
  removeElementInListForKey: function (element, key) {
    //DART: this._data[key].removeAt(index);
    var index = this._data[key].indexOf(element);
    index > -1 ? this._data[key].splice(index, 1) : null;
  },
  lengthOfListForKey: function (key) {
...
ASMFactory.prototype = {
  initialize: function () {
    /*int*/
    this.cacheExpiredTime = ASM_NO_EXPIRATION;
    /*String*/
    this.serverUrl = '';
    /*String*/
    this.authAppId = '';
    ...
    this.authToken = '';
    /*String*/
    this.cacheExpirationTimeKey = 'expirationtime';
    this.cacheTimestampKey = 'timestamp';
    /*String*/
    this.cacheableKey = 'cacheable';
    /*String*/
    this.syncableKey = 'syncableKey';
    ...
    this.appId = fibity.manager.api.appId;  //Si hay token -> consultar token
  ...
  logout: function () {
    ...
    gapi.client.account.logout(request).execute(function (resp) {
      ...
      location.reload();  /*asm.setToken(undefined);
		 	  				if(resp){
		 	  					future.return(true);
		 	  				}else{
		 	  					future.return(false);
  ...
  setExpiredTime: function (cache_expired_time) {
    this.cacheExpiredTime = cache_expired_time;
  },
  setServerAuthentication: function (appId, appKey, Token) {
    this.authAppId = appId;
    this.authAppKey = appKey;
    this.authToken = Token;
  },
  configKind: function (kind, syncable, cacheable, cache_expired_time) {
    var obj = {};
  ...
  getEntityOfKindById: function (kind, id) {
    var future = new asyncFuture();
    /*ASMEntity*/
    var result = null;
    /*String*/
    value = localStorage[kind + ':' + id];
    if (value != null) {
  ...
  getAllEntitiesOfKind: function (kind) {
    /*ASMEntity*/
    var entity;
    /*List<ASMEntity>*/
    var result = [];
    ...
    if (actualTimestamp < timestamp + expiration) {
      ...
      if ([
        ...
        request.organizationId = asm.getCurrentOrganizationId();
  ...
  removeAllEntitiesOfKind: function (kind) {
    for (var key in localStorage) {
      if (key.indexOf(kind + ':') != -1) {
  ...
  save: function (entity) {
    var future = new asyncFuture();
    /*Map*/
    var kindconfig = this.kinds[entity.kind()];
    if (kindconfig != null) {
  ...
  uploadImagesSync: function (files) {
    var future = new asyncFuture();
    request = new XMLHttpRequest();
...
/*Future<List<Antenna>> */
Antenna.fromEntityList = function (asmFuture) {
  var future = new asyncFuture();
...
Antenna.prototype.setName = function (name) {
  this.setStringForKey(name, this.pName);
};
Antenna.prototype.getName = function () {
  return this.stringForKey(this.pName);
};
//Serial is Read only
Antenna.prototype.getSerialNumber = function () {
  return this.stringForKey(this.pSerialNumber);
};
Antenna.prototype.setLocation = function (location) {
  this.setStringForKey(location, this.pLocation);
};
Antenna.prototype.getLocation = function () {
  return this.stringForKey(this.pLocation);
};
Antenna.prototype.setSection = function (section) {
  this.setStringForKey(section, this.pSection);
};
Antenna.prototype.getSection = function () {
  return this.stringForKey(this.pSection);
};
Antenna.prototype.setSchedulesList = function (schedulesList) {
  this.setListForKey(schedulesList, this.pSchedulesList);
};
Antenna.prototype.getSchedulesList = function () {
  return this.listForKey(this.pSchedulesList);
};
Antenna.prototype.addScheduleToList = function (scheduleId) {
  this.addItemToListForKey(scheduleId, this.pSchedulesList);
};
Antenna.prototype.removeScheduleIdInScheduleList = function (scheduleId) {
  this.removeElementInListForKey(scheduleId, this.pSchedulesList);
};
Antenna.prototype.removeIndexInScheduleList = function (index) {
  this.removeIndexInListForKey(index, this.pSchedulesList);
};
//Create Properties
...
Object.defineProperty(Antenna.prototype, 'schedulesList', {
  set: function schedulesList(v) {
    this.setSchedulesList(v);
  },
  get: function schedulesList() {
    return this.getSchedulesList();
  }
});
/*************** End Class Antenna ***************/
...
AntennaActivation.fromEntityList = function (asmFuture) {
  var future = new asyncFuture();
  asmFuture.then(function (list) {
...
//Create Setters and Getters
AntennaActivation.prototype.setName = function (name) {
  this.setStringForKey(name, this.pName);
};
AntennaActivation.prototype.getName = function () {
  return this.stringForKey(this.pName);
};
AntennaActivation.prototype.setLocation = function (location) {
  this.setStringForKey(location, this.pLocation);
};
AntennaActivation.prototype.getLocation = function () {
  return this.stringForKey(this.pLocation);
};
AntennaActivation.prototype.setSection = function (section) {
  this.setStringForKey(section, this.pSection);
};
AntennaActivation.prototype.getSection = function () {
  return this.stringForKey(this.pSection);
};
AntennaActivation.prototype.setKey = function (key) {
  this.setStringForKey(key, this.pKey);
};
AntennaActivation.prototype.getKey = function () {
  return this.stringForKey(this.pKey);
};
//Create Properties
...
var Billing = Class.create(ASMEntity, {
    initialize: function ($super) {
      ...
      this.pDni = 'dni';
      /*String*/
      this.pDireccion1 = 'direccion1';
      /*String*/
      this.pDireccion2 = 'direccion2';
      /*String*/
      this.pPoblacion = 'poblacion';
      /*String*/
      this.pCodigoPostal = 'codigo_postal';
      /*String*/
    ...
    initWithId: function ($super, kind, id, data) {
      //Call parent init method
...
/*Future<List<Billing>> */
Billing.fromEntityList = function (asmFuture) {
  var future = new asyncFuture();
  asmFuture.then(function (list) {
    /*List<AntennaActivation>*/
    var result = [];
...
//Create Setters and Getters
Billing.prototype.setRazonSocial = function (razonSocial) {
  this.setStringForKey(razonSocial, this.pRazonSocial);
};
Billing.prototype.getRazonSocial = function () {
  return this.stringForKey(this.pRazonSocial);
};
Billing.prototype.setCIF = function (cif) {
  this.setStringForKey(cif, this.pCIF);
};
Billing.prototype.getCIF = function () {
  return this.stringForKey(this.pCIF);
};
Billing.prototype.setName = function (name) {
  this.setStringForKey(name, this.pName);
};
Billing.prototype.getName = function () {
  return this.stringForKey(this.pName);
};
Billing.prototype.setApellido = function (apellido) {
  this.setStringForKey(apellido, this.pApellido);
};
Billing.prototype.getApellido = function () {
  return this.stringForKey(this.pApellido);
};
Billing.prototype.setDni = function (dni) {
  this.setStringForKey(dni, this.pDni);
};
Billing.prototype.getDni = function () {
  return this.stringForKey(this.pDni);
};
Billing.prototype.setDireccion1 = function (direccion1) {
  this.setStringForKey(direccion1, this.pDireccion1);
};
Billing.prototype.getDireccion1 = function () {
  return this.stringForKey(this.pDireccion1);
};
Billing.prototype.setDireccion2 = function (direccion2) {
  this.setStringForKey(direccion2, this.pDireccion2);
};
Billing.prototype.getDireccion2 = function () {
  return this.stringForKey(this.pDireccion2);
};
Billing.prototype.setPoblacion = function (poblacion) {
  this.setStringForKey(poblacion, this.pPoblacion);
};
Billing.prototype.getPoblacion = function () {
  return this.stringForKey(this.pPoblacion);
};
Billing.prototype.setCodigoPostal = function (codigoPostal) {
  this.setStringForKey(codigoPostal, this.pCodigoPostal);
};
Billing.prototype.getCodigoPostal = function () {
  return this.stringForKey(this.pCodigoPostal);
};
Billing.prototype.setProvincia = function (provincia) {
  this.setStringForKey(provincia, this.pProvincia);
};
Billing.prototype.getProvincia = function () {
  return this.stringForKey(this.pProvincia);
};
Billing.prototype.setTelefono = function (telefono) {
  this.setStringForKey(telefono, this.pTelefono);
};
Billing.prototype.getTelefono = function () {
  return this.stringForKey(this.pTelefono);
};
//Create Properties
Billing.prototype.razonSocial = '';
Object.defineProperty(Billing.prototype, 'razonSocial', {
  set: function razonSocial(v) {
    this.setRazonSocial(v);
  },
  get: function razonSocial() {
...
Object.defineProperty(Billing.prototype, 'apellido', {
  set: function apellido(v) {
    this.setApellido(v);
  },
  get: function apellido() {
...
var Campaign = Class.create(ASMEntity, {
    initialize: function ($super) {
      /*String*/
      /*String*/
      this.pInfoCardId = 'infocardId';
      /*List*/
      this.pAntennasList = 'antennasList';
      /*String*/
      this.pName = 'name';
      /*Bool*/
      this.pDynamic = 'dynamic';
      /*List*/
      this.pSchedulesList = 'scheduleList';
      //Call parent construct
      $super();
    },
    init: function ($super, data) {
...
Campaign.fromEntityList = function (asmFuture) {
  var future = new asyncFuture();
  asmFuture.then(function (list) {
    var result = [];
    list.forEach(function (e) {
...
//Create Setters and Getters
Campaign.prototype.setInfocardId = function (infocardId) {
  this.setStringForKey(infocardId, this.pInfoCardId);
};
Campaign.prototype.getInfocardId = function () {
  return this.stringForKey(this.pInfoCardId);
};
Campaign.prototype.setAntennasList = function (antennasList) {
  this.setListForKey(antennasList, this.pAntennasList);
};
Campaign.prototype.getAntennasList = function () {
  return this.listForKey(this.pAntennasList);
};
Campaign.prototype.addAntennasToList = function (antennasId) {
  this.addItemToListForKey(antennasId, this.pAntennasList);
};
Campaign.prototype.removeAntennasIdInAntennasList = function (antennasId) {
  this.removeElementInListForKey(antennasId, this.pAntennasList);
};
Campaign.prototype.removeIndexInAntennasList = function (index) {
  this.removeIndexInListForKey(index, this.pAntennasList);
};
...
Campaign.prototype.setDynamic = function (dynamic) {
  this.setBoolForKey(dynamic, this.pDynamic);
};
Campaign.prototype.getDynamic = function () {
  return this.boolForKey(this.pDynamic);
};
Campaign.prototype.setSchedulesList = function (schedulesList) {
  this.setListForKey(schedulesList, this.pSchedulesList);
};
Campaign.prototype.getSchedulesList = function () {
  return this.listForKey(this.pSchedulesList);
};
Campaign.prototype.addScheduleToList = function (scheduleId) {
  this.addItemToListForKey(scheduleId, this.pSchedulesList);
};
Campaign.prototype.removeScheduleIdInScheduleList = function (scheduleId) {
  this.removeElementInListForKey(scheduleId, this.pSchedulesList);
};
Campaign.prototype.removeIndexInScheduleList = function (index) {
  this.removeIndexInListForKey(index, this.pSchedulesList);
};
//Create Properties
Campaign.prototype.infocardId = '';
Object.defineProperty(Campaign.prototype, 'infocardId', {
  set: function infocardId(v) {
    this.setInfocardId(v);
  },
  get: function infocardId() {
    return this.getInfocardId();
  }
});
Campaign.prototype.dynamicMode = '';
Object.defineProperty(Campaign.prototype, 'dynamicMode', {
  set: function dynamicMode(v) {
    this.setDynamic(v);
  },
  get: function dynamicMode() {
    return this.getDynamic();
  }
});
Campaign.prototype.antennasList = [];
Object.defineProperty(Campaign.prototype, 'antennasList', {
  set: function antennasList(v) {
    this.setAntennasList(v);
  },
  get: function antennasList() {
    return this.getAntennasList();
  }
});
...
Customers.fromEntityList = function (asmFuture) {
  var future = new asyncFuture();
  asmFuture.then(function (list) {
    /*List<Customers>*/
    var result = [];
...
//Create Setters and Getters
Customers.prototype.setName = function (name) {
  this.setStringForKey(name, this.pName);
};
Customers.prototype.getName = function () {
  return this.stringForKey(this.pName);
};
Customers.prototype.setApellido = function (apellido) {
  this.setStringForKey(apellido, this.pApellido);
};
Customers.prototype.getApellido = function () {
  return this.stringForKey(this.pApellido);
};
Customers.prototype.setFoto = function (foto) {
  this.setStringForKey(foto, this.pFoto);
};
Customers.prototype.getFoto = function () {
  return this.stringForKey(this.pFoto);
};
Customers.prototype.setPuntos = function (puntos) {
  this.setIntForKey(puntos, this.pPuntos);
};
Customers.prototype.getPuntos = function () {
  return this.intForKey(this.pPuntos);
};
Customers.prototype.setLastVisita = function (lastVisita) {
  this.setStringForKey(lastVisita, this.pLastVisita);
};
Customers.prototype.getLastVisita = function () {
  return this.stringForKey(this.pLastVisita);
};
//Create Properties
...
Object.defineProperty(Customers.prototype, 'lastVisita', {
  set: function lastVisita(v) {
    this.setLastVisita(v);
  },
  get: function lastVisita() {
    return this.getLastVisita();
  }
});
/*************** End Class Customers ***************/
/********************	Class Infocard   ******************/
...
/*Future<List<Antenna>> */
InfoCard.fromEntityList = function (asmFuture) {
  var future = new asyncFuture();
  asmFuture.then(function (list) {
    /*List<Antenna>*/
    var result = [];
...
//Create Setters and Getters
InfoCard.prototype.setUrlImg = function (urlImg) {
  this.setStringForKey(urlImg, this.pUrlImg);
};
InfoCard.prototype.getUrlImg = function () {
  return this.stringForKey(this.pUrlImg);
};
InfoCard.prototype.setTitle = function (title) {
  this.setStringForKey(title, this.pTitle);
};
InfoCard.prototype.getTitle = function () {
  return this.stringForKey(this.pTitle);
};
InfoCard.prototype.setDescription = function (description) {
  this.setStringForKey(description, this.pDescription);
};
InfoCard.prototype.getDescription = function () {
  return this.stringForKey(this.pDescription);
};
//Create Properties
InfoCard.prototype.urlImg = '';
Object.defineProperty(InfoCard.prototype, 'urlImg', {
  set: function urlImg(v) {
    this.setUrlImg(v);
  },
  get: function urlImg() {
...
var Organization = Class.create(ASMEntity, {
    initialize: function ($super) {
      /*String*/
      this.pUrlImgLogo = 'urlImgLogo';
      /*String*/
      this.pUrlImgBackground = 'urlImgBackground';
      /*String*/
      this.pName = 'name';
      /*String*/
      this.pSlogan = 'slogan';
...
/*Future<List<Antenna>> */
Organization.fromEntityList = function (asmFuture) {
  var future = new asyncFuture();
  asmFuture.then(function (list) {
    /*List<Antenna>*/
    var result = [];
...
//Create Setters and Getters
Organization.prototype.setUrlImgLogo = function (urlImgLogo) {
  this.setStringForKey(urlImgLogo, this.pUrlImgLogo);
};
Organization.prototype.getUrlImgLogo = function () {
  return this.stringForKey(this.pUrlImgLogo);
};
Organization.prototype.setUrlImgBackground = function (urlImgBackground) {
  this.setStringForKey(urlImgBackground, this.pUrlImgBackground);
};
Organization.prototype.getUrlImgBackground = function () {
  return this.stringForKey(this.pUrlImgBackground);
};
...
Organization.prototype.setSlogan = function (slogan) {
  this.setStringForKey(slogan, this.pSlogan);
};
Organization.prototype.getSlogan = function () {
  return this.stringForKey(this.pSlogan);
};
Organization.prototype.setDescription = function (description) {
  this.setStringForKey(description, this.pDescription);
};
Organization.prototype.getDescription = function () {
  return this.stringForKey(this.pDescription);
};
//Create Properties
Organization.prototype.urlImgLogo = '';
Object.defineProperty(Organization.prototype, 'urlImgLogo', {
  set: function urlImgLogo(v) {
    this.setUrlImgLogo(v);
  },
  get: function urlImgLogo() {
    return this.getUrlImgLogo();
  }
});
Organization.prototype.urlImgBackground = '';
Object.defineProperty(Organization.prototype, 'urlImgBackground', {
  set: function urlImgBackground(v) {
    this.setUrlImgBackground(v);
  },
  get: function urlImgBackground() {
    return this.getUrlImgBackground();
  }
});
...
var Schedule = Class.create(ASMEntity, {
    initialize: function ($super) {
      /* FECHAS Y HORAS */
      /*String*/
      this.pDateIni = 'dateIni';
      /*String*/
      this.pDateEnd = 'dateEnd';
      /*String*/
      this.pHourIni = 'hourEni';
      /*String*/
      this.pHourEnd = 'hourEnd';
      /*String*/
      this.pName = 'name';
      /* ID DE INFOCARD Y LISTADO DE ANTENAS */
      /*String*/
      this.pIdinfocard = 'Idinfocard';
      /*String*/
      this.pMaxUser = 'maxUser';
      /*String*/
      this.pAntennasList = 'antennasList';
      /* REPETICION */
      /*String*/
      this.pRepetition = 'repetition';
      /*String*/
      this.pEveryDaily = 'every_daily';
      /*String*/
      this.pEveryWeekly = 'every_weekly';
      /*String*/
      this.pEveryMonthly = 'every_monthly';
      /*String*/
      this.pEveryYearly = 'every_yearly';
      /*String*/
      this.pEveryExactlySelection = 'every_exactly_selection';
      /*String*/
      this.pEveryExactlyDay = 'every_exactly_day';
      /*String*/
      this.pEvery = 'allDay';
      /* END && STATIC */
      /*String*/
      ...
      this.pMonday = 'monday';
      /*String*/
      this.pTuesday = 'tuesday';
      /*String*/
      this.pWednesday = 'wednesday';
      /*String*/
      this.pThursday = 'thursday';
      /*String*/
      this.pFriday = 'friday';
      /*String*/
      this.pSaturday = 'saturday';
      /*String*/
      this.pSunday = 'sunday';
      /* MESES DEL ANYO */
      /*String*/
      this.pJanuary = 'january';
      /*String*/
      this.pFebruary = 'february';
      /*String*/
      this.pMarch = 'march';
      /*String*/
      this.pApril = 'april';
      ...
      this.pAugust = 'august';
      /*String*/
      this.pSeptember = 'september';
      /*String*/
      this.pOctober = 'october';
      /*String*/
      this.pNovember = 'november';
      /*String*/
      this.pDecember = 'december';
      //Call parent constructor
      $super();
    },
    init: function ($super, data) {
...
/*Future<List<Antenna>> */
Schedule.fromEntityList = function (asmFuture) {
  var future = new asyncFuture();
  asmFuture.then(function (list) {
    /*List<Antenna>*/
    var result = [];
...
//Create Setters and Getters
Schedule.prototype.setDateIni = function (dateIni) {
  this.setStringForKey(dateIni, this.pDateIni);
};
Schedule.prototype.getDateIni = function () {
  return this.stringForKey(this.pDateIni);
};
Schedule.prototype.setDateEnd = function (dateEnd) {
  this.setStringForKey(dateEnd, this.pDateEnd);
};
Schedule.prototype.getDateEnd = function () {
  return this.stringForKey(this.pDateEnd);
};
Schedule.prototype.setHourIni = function (hourIni) {
  this.setStringForKey(hourIni, this.pHourIni);
};
Schedule.prototype.getHourIni = function () {
  return this.stringForKey(this.pHourIni);
};
Schedule.prototype.setHourEnd = function (hourEnd) {
  this.setStringForKey(hourEnd, this.pHourEnd);
};
Schedule.prototype.getHourEnd = function () {
  return this.stringForKey(this.pHourEnd);
};
...
Schedule.prototype.setIdinfocard = function (Idinfocard) {
  this.setStringForKey(Idinfocard, this.pIdinfocard);
};
Schedule.prototype.getIdinfocard = function () {
  return this.stringForKey(this.pIdinfocard);
};
Schedule.prototype.setMaxUser = function (maxUser) {
  this.setIntForKey(maxUser, this.pMaxUser);
};
Schedule.prototype.getMaxUser = function () {
  return this.intForKey(this.pMaxUser);
};
Schedule.prototype.setAntennasList = function (AntennasList) {
  this.setListForKey(AntennasList, this.pAntennasList);
};
Schedule.prototype.getAntennasList = function () {
  return this.listForKey(this.pAntennasList);
};
Schedule.prototype.addAntennasToList = function (AntennasId) {
  this.addItemToListForKey(AntennasId, this.pAntennasList);
};
Schedule.prototype.removeAntennasIdInAntennasList = function (AntennasId) {
  this.removeElementInListForKey(AntennasId, this.pAntennasList);
};
Schedule.prototype.removeIndexInAntennasList = function (index) {
  this.removeIndexInListForKey(index, this.pAntennasList);
};
/* REPETICION */
Schedule.prototype.setRepetition = function (repetition) {
  this.setIntForKey(repetition, this.pRepetition);
};
Schedule.prototype.getRepetition = function () {
  return this.intForKey(this.pRepetition);
};
Schedule.prototype.setEveryDaily = function (everyDaily) {
  this.setIntForKey(everyDaily, this.pEveryDaily);
};
Schedule.prototype.getEveryDaily = function () {
  return this.intForKey(this.pEveryDaily);
};
Schedule.prototype.setEveryWeekly = function (everyWeekly) {
  this.setIntForKey(everyWeekly, this.pEveryWeekly);
};
Schedule.prototype.getEveryWeekly = function () {
  return this.intForKey(this.pEveryWeekly);
};
Schedule.prototype.setEveryMonthly = function (everyMonthly) {
  this.setIntForKey(everyMonthly, this.pEveryMonthly);
};
Schedule.prototype.getEveryMonthly = function () {
  return this.intForKey(this.pEveryMonthly);
};
Schedule.prototype.setEveryYearly = function (everyYearly) {
  this.setIntForKey(everyYearly, this.pEveryYearly);
};
Schedule.prototype.getEveryYearly = function () {
  return this.intForKey(this.pEveryYearly);
};
Schedule.prototype.setEveryExactlySelection = function (everyExactlySelection) {
  this.setIntForKey(everyExactlySelection, this.pEveryExactlySelection);
};
Schedule.prototype.getEveryExactlySelection = function () {
  return this.intForKey(this.pEveryExactlySelection);
};
Schedule.prototype.setEveryExactlyDay = function (everyExactlyDay) {
  this.setIntForKey(everyExactlyDay, this.pEveryExactlyDay);
};
Schedule.prototype.getEveryExactlyDay = function () {
  return this.intForKey(this.pEveryExactlyDay);
};
Schedule.prototype.setEvery = function (every) {
  this.setIntForKey(every, this.pEvery);
};
Schedule.prototype.getEvery = function () {
  return this.intForKey(this.pEvery);
};
/**	FIN **/
/*** END && STATIC ***/
Schedule.prototype.setEnd = function (end) {
  this.setIntForKey(end, this.pEnd);
};
Schedule.prototype.getEnd = function () {
  return this.intForKey(this.pEnd);
};
Schedule.prototype.setStatico = function (statico) {
  this.setBoolForKey(statico, this.pStatico);
};
Schedule.prototype.getStatico = function () {
  return this.boolForKey(this.pStatico);
};
/*********DIAS**********/
Schedule.prototype.setDay1 = function (day1) {
  this.setBoolForKey(day1, this.pDay1);
};
Schedule.prototype.getDay1 = function () {
  return this.boolForKey(this.pDay1);
};
Schedule.prototype.setDay2 = function (day2) {
  this.setBoolForKey(day2, this.pDay2);
};
Schedule.prototype.getDay2 = function () {
  return this.boolForKey(this.pDay2);
};
Schedule.prototype.setDay3 = function (day3) {
  this.setBoolForKey(day3, this.pDay3);
};
Schedule.prototype.getDay3 = function () {
  return this.boolForKey(this.pDay3);
};
Schedule.prototype.setDay4 = function (day4) {
  this.setBoolForKey(day4, this.pDay4);
};
Schedule.prototype.getDay4 = function () {
  return this.boolForKey(this.pDay4);
};
Schedule.prototype.setDay5 = function (day5) {
  this.setBoolForKey(day5, this.pDay5);
};
Schedule.prototype.getDay5 = function () {
  return this.boolForKey(this.pDay5);
};
Schedule.prototype.setDay6 = function (day6) {
  this.setBoolForKey(day6, this.pDay6);
};
Schedule.prototype.getDay6 = function () {
  return this.boolForKey(this.pDay6);
};
Schedule.prototype.setDay7 = function (day7) {
  this.setBoolForKey(day7, this.pDay7);
};
Schedule.prototype.getDay7 = function () {
  return this.boolForKey(this.pDay7);
};
Schedule.prototype.setDay8 = function (day8) {
  this.setBoolForKey(day8, this.pDay8);
};
Schedule.prototype.getDay8 = function () {
  return this.boolForKey(this.pDay8);
};
Schedule.prototype.setDay9 = function (day9) {
  this.setBoolForKey(day9, this.pDay9);
};
Schedule.prototype.getDay9 = function () {
  return this.boolForKey(this.pDay9);
};
Schedule.prototype.setDay10 = function (day10) {
  this.setBoolForKey(day10, this.pDay10);
};
Schedule.prototype.getDay10 = function () {
  return this.boolForKey(this.pDay10);
};
Schedule.prototype.setDay11 = function (day11) {
  this.setBoolForKey(day11, this.pDay11);
};
Schedule.prototype.getDay11 = function () {
  return this.boolForKey(this.pDay11);
};
Schedule.prototype.setDay12 = function (day12) {
  this.setBoolForKey(day12, this.pDay12);
};
Schedule.prototype.getDay12 = function () {
  return this.boolForKey(this.pDay12);
};
Schedule.prototype.setDay13 = function (day13) {
  this.setBoolForKey(day13, this.pDay13);
};
Schedule.prototype.getDay13 = function () {
  return this.boolForKey(this.pDay13);
};
Schedule.prototype.setDay14 = function (day14) {
  this.setBoolForKey(day14, this.pDay14);
};
Schedule.prototype.getDay14 = function () {
  return this.boolForKey(this.pDay14);
};
Schedule.prototype.setDay15 = function (day15) {
  this.setBoolForKey(day15, this.pDay15);
};
Schedule.prototype.getDay15 = function () {
  return this.boolForKey(this.pDay15);
};
Schedule.prototype.setDay16 = function (day16) {
  this.setBoolForKey(day16, this.pDay16);
};
Schedule.prototype.getDay16 = function () {
  return this.boolForKey(this.pDay16);
};
Schedule.prototype.setDay17 = function (day17) {
  this.setBoolForKey(day17, this.pDay17);
};
Schedule.prototype.getDay17 = function () {
  return this.boolForKey(this.pDay17);
};
Schedule.prototype.setDay18 = function (day18) {
  this.setBoolForKey(day18, this.pDay18);
};
Schedule.prototype.getDay18 = function () {
  return this.boolForKey(this.pDay18);
};
Schedule.prototype.setDay19 = function (day19) {
  this.setBoolForKey(day19, this.pDay19);
};
Schedule.prototype.getDay19 = function () {
  return this.boolForKey(this.pDay19);
};
Schedule.prototype.setDay20 = function (day20) {
  this.setBoolForKey(day20, this.pDay20);
};
Schedule.prototype.getDay20 = function () {
  return this.boolForKey(this.pDay20);
};
Schedule.prototype.setDay21 = function (day21) {
  this.setBoolForKey(day21, this.pDay21);
};
Schedule.prototype.getDay21 = function () {
  return this.boolForKey(this.pDay21);
};
Schedule.prototype.setDay22 = function (day22) {
  this.setBoolForKey(day22, this.pDay22);
};
Schedule.prototype.getDay22 = function () {
  return this.boolForKey(this.pDay22);
};
Schedule.prototype.setDay23 = function (day23) {
  this.setBoolForKey(day23, this.pDay23);
};
Schedule.prototype.getDay23 = function () {
  return this.boolForKey(this.pDay23);
};
Schedule.prototype.setDay24 = function (day24) {
  this.setBoolForKey(day24, this.pDay24);
};
Schedule.prototype.getDay24 = function () {
  return this.boolForKey(this.pDay24);
};
Schedule.prototype.setDay25 = function (day25) {
  this.setBoolForKey(day25, this.pDay25);
};
Schedule.prototype.getDay25 = function () {
  return this.boolForKey(this.pDay25);
};
Schedule.prototype.setDay26 = function (day26) {
  this.setBoolForKey(day26, this.pDay26);
};
Schedule.prototype.getDay26 = function () {
  return this.boolForKey(this.pDay26);
};
Schedule.prototype.setDay27 = function (day27) {
  this.setBoolForKey(day27, this.pDay27);
};
Schedule.prototype.getDay27 = function () {
  return this.boolForKey(this.pDay27);
};
Schedule.prototype.setDay28 = function (day28) {
  this.setBoolForKey(day28, this.pDay28);
};
Schedule.prototype.getDay28 = function () {
  return this.boolForKey(this.pDay28);
};
Schedule.prototype.setDay29 = function (day29) {
  this.setBoolForKey(day29, this.pDay29);
};
Schedule.prototype.getDay29 = function () {
  return this.boolForKey(this.pDay29);
};
Schedule.prototype.setDay30 = function (day30) {
  this.setBoolForKey(day30, this.pDay30);
};
Schedule.prototype.getDay30 = function () {
  return this.boolForKey(this.pDay30);
};
Schedule.prototype.setDay31 = function (day31) {
  this.setBoolForKey(day31, this.pDay31);
};
Schedule.prototype.getDay31 = function () {
  return this.boolForKey(this.pDay31);
};
/*************** dias de la semana *****************/
Schedule.prototype.setMonday = function (monday) {
  this.setBoolForKey(monday, this.pMonday);
};
Schedule.prototype.getMonday = function () {
  return this.boolForKey(this.pMonday);
};
Schedule.prototype.setTuesday = function (tuesday) {
  this.setBoolForKey(tuesday, this.pTuesday);
};
Schedule.prototype.getTuesday = function () {
  return this.boolForKey(this.pTuesday);
};
Schedule.prototype.setWednesday = function (wednesday) {
  this.setBoolForKey(wednesday, this.pWednesday);
};
Schedule.prototype.getWednesday = function () {
  return this.boolForKey(this.pWednesday);
};
Schedule.prototype.setThursday = function (thursday) {
  this.setBoolForKey(thursday, this.pThursday);
};
Schedule.prototype.getThursday = function () {
  return this.boolForKey(this.pThursday);
};
Schedule.prototype.setFriday = function (friday) {
  this.setBoolForKey(friday, this.pFriday);
};
Schedule.prototype.getFriday = function () {
  return this.boolForKey(this.pFriday);
};
Schedule.prototype.setSaturday = function (saturday) {
  this.setBoolForKey(saturday, this.pSaturday);
};
Schedule.prototype.getSaturday = function () {
  return this.boolForKey(this.pSaturday);
};
Schedule.prototype.setSunday = function (sunday) {
  this.setBoolForKey(sunday, this.pSunday);
};
Schedule.prototype.getSunday = function () {
  return this.boolForKey(this.pSunday);
};
/********************* nombre de los meses **************************/
Schedule.prototype.setJanuary = function (january) {
  this.setBoolForKey(january, this.pJanuary);
};
Schedule.prototype.getJanuary = function () {
  return this.boolForKey(this.pJanuary);
};
Schedule.prototype.setFebruary = function (february) {
  this.setBoolForKey(february, this.pFebruary);
};
Schedule.prototype.getFebruary = function () {
  return this.boolForKey(this.pFebruary);
};
Schedule.prototype.setMarch = function (march) {
  this.setBoolForKey(march, this.pMarch);
};
Schedule.prototype.getMarch = function () {
  return this.boolForKey(this.pMarch);
};
Schedule.prototype.setApril = function (april) {
  this.setBoolForKey(april, this.pApril);
};
Schedule.prototype.getApril = function () {
  return this.boolForKey(this.pApril);
};
Schedule.prototype.setMay = function (may) {
  this.setBoolForKey(may, this.pMay);
};
Schedule.prototype.getMay = function () {
  return this.boolForKey(this.pMay);
};
Schedule.prototype.setJune = function (june) {
  this.setBoolForKey(june, this.pJune);
};
Schedule.prototype.getJune = function () {
  return this.boolForKey(this.pJune);
};
Schedule.prototype.setJuly = function (july) {
  this.setBoolForKey(july, this.pJuly);
};
Schedule.prototype.getJuly = function () {
  return this.boolForKey(this.pJuly);
};
Schedule.prototype.setAugust = function (august) {
  this.setBoolForKey(august, this.pAugust);
};
Schedule.prototype.getAugust = function () {
  return this.boolForKey(this.pAugust);
};
Schedule.prototype.setSeptember = function (september) {
  this.setBoolForKey(september, this.pSeptember);
};
Schedule.prototype.getSeptember = function () {
  return this.boolForKey(this.pSeptember);
};
Schedule.prototype.setOctober = function (october) {
  this.setBoolForKey(october, this.pOctober);
};
Schedule.prototype.getOctober = function () {
  return this.boolForKey(this.pOctober);
};
Schedule.prototype.setNovember = function (november) {
  this.setBoolForKey(november, this.pNovember);
};
Schedule.prototype.getNovember = function () {
  return this.boolForKey(this.pNovember);
};
Schedule.prototype.setDecember = function (december) {
  this.setBoolForKey(december, this.pDecember);
};
Schedule.prototype.getDecember = function () {
  return this.boolForKey(this.pDecember);
};
/********************* fin nombre de los meses **************************/
//Create Properties
Schedule.prototype.dateIni = '';
Object.defineProperty(Schedule.prototype, 'dateIni', {
  set: function dateIni(v) {
    this.setDateIni(v);
  },
  get: function dateIni() {
    return this.getDateIni();
  }
});
Schedule.prototype.dateEnd = '';
Object.defineProperty(Schedule.prototype, 'dateEnd', {
  set: function dateEnd(v) {
    this.setDateEnd(v);
  },
  get: function dateEnd() {
    return this.getDateEnd();
  }
});
Schedule.prototype.hourIni = '';
Object.defineProperty(Schedule.prototype, 'hourIni', {
  set: function hourIni(v) {
    this.setHourIni(v);
  },
  get: function hourIni() {
    return this.getHourIni();
  }
});
Schedule.prototype.hourEnd = '';
Object.defineProperty(Schedule.prototype, 'hourEnd', {
  set: function hourEnd(v) {
    this.setHourEnd(v);
  },
  get: function hourEnd() {
...
Object.defineProperty(Schedule.prototype, 'Idinfocard', {
  set: function Idinfocard(v) {
    this.setIdinfocard(v);
  },
  get: function Idinfocard() {
    return this.getIdinfocard();
  }
});
Schedule.prototype.maxUser = '';
Object.defineProperty(Schedule.prototype, 'maxUser', {
  set: function maxUser(v) {
    this.setMaxUser(v);
  },
  get: function maxUser() {
...
Object.defineProperty(Schedule.prototype, 'repetition', {
  set: function repetition(v) {
    this.setRepetition(v);
  },
  get: function repetition() {
    return this.getRepetition();
  }
});
Schedule.prototype.everyDaily = '';
Object.defineProperty(Schedule.prototype, 'everyDaily', {
  set: function everyDaily(v) {
    this.setEveryDaily(v);
  },
  get: function everyDaily() {
    return this.getEveryDaily();
  }
});
Schedule.prototype.everyWeekly = '';
Object.defineProperty(Schedule.prototype, 'everyWeekly', {
  set: function everyWeekly(v) {
    this.setEveryWeekly(v);
  },
  get: function everyWeekly() {
    return this.getEveryWeekly();
  }
});
Schedule.prototype.everyMonthly = '';
Object.defineProperty(Schedule.prototype, 'everyMonthly', {
  set: function everyMonthly(v) {
    this.setEveryMonthly(v);
  },
  get: function everyMonthly() {
    return this.getEveryMonthly();
  }
});
Schedule.prototype.everyYearly = '';
Object.defineProperty(Schedule.prototype, 'everyYearly', {
  set: function everyYearly(v) {
    this.setEveryYearly(v);
  },
  get: function everyYearly() {
    return this.getEveryYearly();
  }
});
Schedule.prototype.everyExactlySelection = '';
Object.defineProperty(Schedule.prototype, 'everyExactlySelection', {
  set: function everyExactlySelection(v) {
    this.setEveryExactlySelection(v);
  },
  get: function everyExactlySelection() {
    return this.getEveryExactlySelection();
  }
});
Schedule.prototype.everyExactlyDay = '';
Object.defineProperty(Schedule.prototype, 'everyExactlyDay', {
  set: function everyExactlyDay(v) {
    this.setEveryExactlyDay(v);
  },
  get: function everyExactlyDay() {
    return this.getEveryExactlyDay();
  }
});
Schedule.prototype.every = '';
Object.defineProperty(Schedule.prototype, 'every', {
  set: function every(v) {
    this.setEvery(v);
  },
  get: function every() {
...
Object.defineProperty(Schedule.prototype, 'statico', {
  set: function statico(v) {
    this.setStatico(v);
  },
  get: function statico() {
    return this.getStatico();
  }
});
/** DIAS **/
...
// directiva del focus	
angular.module('ng').directive('ngFocus', [
  '$timeout',
  function ($timeout) {
    return {
      link: function (scope, element, attrs) {
...
fibity.manager.app.directive('fbtmTopbar', function () {
  ...
  directive.templateUrl = 'views/components/fbtmTopbarComponent.html', directive.scope = {
    profile: '=',
    path: '='
  };
  return directive;
...
fibity.manager.app.directive('fbtmSidebar', function () {
  ...
  directive.restrict = 'E';
  /* restrict this directive to elements */
  directive.templateUrl = 'views/components/fbtmSidebarComponent.html', directive.scope = { active: '=activeitem' };
  directive.link = function (scope) {
    scope.options = [
      ...
      },
      {
        'id': 'antennas',
        'title': 'Antenas',
        'cssclass': 'glyphicon glyphicon-map-marker'
      },
      {
        'id': 'infocard',
        'title': 'Contenido',
      ...
      },
      {
        'id': 'campaigns',
        'title': 'Campa\xf1as',
        'cssclass': 'glyphicon glyphicon-calendar'
      },
      {
        'id': 'customers',
        'title': 'Clientes',
        'cssclass': 'glyphicon glyphicon-user'
      },
      {
        'id': 'billing',
        'title': 'Facturaci\xf3n',
        'cssclass': 'glyphicon glyphicon-credit-card'
      }
    ];
  };
  return directive;
...
fibity.manager.app.controller('lineCtrl', function ($scope) {
  $scope.lineData = {
    labels: [
      'January',
      'February',
...
fibity.manager.app.controller('barCtrl', function ($scope) {
  $scope.barData = {
    labels: [
      'January',
      'February',
...
fibity.manager.app.directive('swicthFibity', function () {
  ...
  directive.scope = { profile: '=profile' };
  return directive;
});
fibity.manager.app.directive('donutChart', function () {
  function link(scope, el, attr) {
    ...
    scope.$watch('data', function (data) {
      ...
      arcs.enter().append('path').style('stroke', 'white').attr('fill', function (d, i) {
        return color(i);
      });
      arcs.exit().remove();
      arcs.attr('d', arc);
...
fibity.manager.app.directive('fbtmInputFileLoader', [
  'fileReader',
  function (fileReader) {
    var directive = {};
    directive.restrict = 'E';
...
fibity.manager.app.controller('GTLoginVC', [
  '$scope',
  '$http',
  '$route',
    ...
    $scope.doLogin = function () {
      ...
      asm.login($scope.email, $scope.password).then(function (processOK) {
        if (processOK) {
          ...
          console.log('usuario y constrase\xf1a incorrecta!');
          $scope.errorMsg = true;
        }
        $scope.$apply();
      }).done();
...
fibity.manager.app.controller('GTLogoutVC', [
  '$scope',
  '$http',
  '$route',
  function ($scope, $http, $route) {
...
fibity.manager.app.controller('GTDashboardVC', [
  '$scope',
  '$routeParams',
  function ($scope, $routeParams) {
    ...
    $scope.activeitem = 'dashboard';  //$scope.param = $routeParams.param;
  }
]);
fibity.manager.app.controller('GTAntennasVC', [
  '$scope',
  '$routeParams',
  '$modal',
  function ($scope, $routeParams, $modal) {
    ...
    $scope.listCab = [
      'Nombre',
      'Lugar de instalaci\xf3n',
      'Departamento/Secci\xf3n',
      'Editar'
    ];
    /* Mapa  */
    $scope.antennaList = {};
    ...
    // Lista de ids de los objetos antenna del modelo
    Antenna.fromEntityList(asm.getAllEntitiesOfKind(Antenna.entityKind)).then(function (list) {
    ...
    $scope.openAntennaModal = function (id) {
      ...
      modalInstance.result.then(function (tmpAntenna) {
        tmpAntenna.save().then(function (mapa) {
...
fibity.manager.app.controller('GTAntennasMapVC', [
  '$scope',
  '$routeParams',
  '$modal',
  function ($scope, $routeParams, $modal) {
...
fibity.manager.app.controller('GTAntennasAddModalVC', [
  '$scope',
  '$modalInstance',
  'tmpAntenna',
  function ($scope, $modalInstance, tmpAntenna) {
    $scope.tmpAntenna = {};
    $scope.activation = {};
    /*String*/
    $scope.activation.txtcasilla1 = '';
    /*String*/
    $scope.activation.txtcasilla2 = '';
    /*String*/
    $scope.activation.txtcasilla3 = '';
    /*String*/
    $scope.activation.txtcasilla4 = '';
    /*bool*/
    $scope.ca1 = false;
    /*bool*/
    $scope.ca2 = false;
    ...
    $scope.cambio = false;
    /*bool*/
    $scope.txtfocus1 = true;
    /*bool*/
    $scope.txtfocus2 = false;
    /*bool*/
    $scope.txtfocus3 = false;
    /*bool*/
    $scope.txtfocus4 = false;
    /*bool*/
    $scope.txtfocus5 = false;
    /*bool*/
    $scope.txtfocus6 = false;
    /*String*/
    $scope.urlImgOrgBackground = '';
    asm.getEntityOfKindById(Organization.entityKind, 'myOrganization').then(function (entity) {
      if (entity != null) {
...
fibity.manager.app.controller('GTBillingVC', [
  '$scope',
  '$routeParams',
  '$modal',
  function ($scope, $routeParams, $modal) {
    ...
    for (var i = 0; i < $scope.periodo.length; i++) {
      ...
      $scope.factura.push({
        'periodo': $scope.periodo[i],
        'cuota': $scope.cuota,
        'consumo': rng1,
        'iva': $scope.iva.toFixed(2),
        'total': $scope.total.toFixed(2)
      });
    }
    // fin de la creacion de la factura
    $scope.billingId = 'MyBilling';
    asm.getEntityOfKindById(Billing.entityKind, $scope.billingId).then(function (entity) {
      if (entity == null) {
...
fibity.manager.app.controller('GTBillingAddModalVC', [
  '$scope',
  '$modalInstance',
  'tmpbilling',
  function ($scope, $modalInstance, tmpbilling) {
    /******** Profile for TopBar Component *******/
    $scope.profile = {
      'name': '',
      'lastname': '',
    ...
    // placeholders
    /*String*/
    $scope.plcrs = 'Raz\xf3n Social';
    /*String*/
    $scope.plccif = 'CIF';
    /*String*/
    $scope.plcnb = 'Nombre';
    /*String*/
    $scope.plcap = 'Apellidos';
    /*String*/
    $scope.plcdni = '00000000A';
    /*String*/
    $scope.plcnif = 'X0000000A';
    //ambos
    /*String*/
    $scope.plcdir1 = 'av. Orovilla 54';
    /*String*/
    $scope.plcdir2 = 'Direccion Opcional';
    /*String*/
    $scope.plcpob = 'Madrid';
    /*String*/
    $scope.plctel = '910000000';
    /*String*/
    $scope.plccp = '28041';
    /*String*/
    $scope.plcpro = 'Provincia';
    // fin
    //autonomo    
    /*String*/
    $scope.prov = [
    ...
    $scope.bmNF = false;
    $scope.boton = true;
    if ($scope.tmpbilling.razonSocil == '' && $scope.tmpbilling.cif == '' && $scope.tmpbilling.direccion1 == '' && $scope.tmpbilling.poblacion == '' && $scope.tmpbilling.codigoPostal == '' && $scope.tmpbilling.provincia == '' && $scope.tmpbilling.telefono == '') {
      $scope.boton = true;
    } else {
      if ($scope.tmpbilling.name == '' && $scope.tmpbilling.apellido == '' && $scope.tmpbilling.dni == '' && $scope.tmpbilling.direccion1 == '' && $scope.tmpbilling.poblacion == '' && $scope.tmpbilling.codigoPostal == '' && $scope.tmpbilling.provincia == '' && $scope.tmpbilling.telefono == '') {
        $scope.boton = true;
      } else {
        $scope.boton = false;
      }
...
fibity.manager.app.controller('GTOrganizationsVC', [
  '$scope',
  '$http',
  function ($scope, $http) {
    /******** Profile for TopBar Component *******/
    ...
    $scope.path = [{
        'name': 'Organizaci\xf3n',
        'href': '#/organization'
      }];
    /********************************************/
    /*String*/
    $scope.title = 'Organizaci\xf3n';
    /*Atributo*/
    $scope.activeitem = 'organization';
    /* bool */
    $scope.boton = true;
    /* bool */
    $scope.btn1 = false;
    ...
    /* bool Img */
    $scope.bmImgLogo = false;
    /* bool Img */
    $scope.bmImgBack = false;
    /*Img Loading*/
    $scope.loading = 'images/36.gif';
    /* ATRIBUTOS DE ESPEJO */
    /*String*/
    $scope.txtEspNombre = 'Bar Mauricio';
    /*String*/
    $scope.txtEspslogan = 'Desayuno 0.99\u20ac';
    /*String*/
    $scope.Organizationid = asm.getCurrentOrganizationId();
    /*String*/
    $scope.tmpOrganization = new Organization();
    ...
    $scope.guardarCambios = function () {
      ...
      if ($scope.bmImgLogo)
        files.push($scope.fileLogo);
      if ($scope.bmImgBack)
        files.push($scope.fileBackground);
      ...
      if (files.length > 0) {
        ...
        asm.uploadImages(files).then(function (res) {
          ...
          if ($scope.bmImgLogo)
            $scope.tmpOrganization.urlImgLogo = res[$scope.fileLogo.name];
          if ($scope.bmImgBack)
            $scope.tmpOrganization.urlImgBackground = res[$scope.fileBackground.name];
...
fibity.manager.app.controller('GTInfoCardsVC', [
  '$scope',
  '$http',
  '$routeParams',
  '$modal',
  function ($scope, $http, $routeParams, $modal) {
    ...
    $scope.path = [{
        'name': 'Tartejas de informaci\xf3n',
        'href': '#/infocard/collection'
      }];
    /**
	 * ******************************************/
    $scope.title = 'Tarjeta de Informaci\xf3n';
    ...
    $scope.infoCardList = {};
    /* List<String> */
    $scope.infoCardKeyList = [];
    $scope.functionInfocardEntity = function () {
      InfoCard.fromEntityList(asm.getAllEntitiesOfKind(InfoCard.entityKind)).then(function (list) {
    ...
    $scope.openInfocardModal = function (id) {
      ...
      modalInstance.result.then(function (tmpInfocard) {
        tmpInfocard.save().then(function (mapa) {
...
fibity.manager.app.controller('GTInfoCardAddModalVC', [
  '$scope',
  '$scope',
  'tmpInfocard',
  'fileReader',
  function ($scope, $scope, tmpInfocard, fileReader) {
    $scope.tmpInfocard = {};
    if (tmpInfocard.id != '') {
      ...
      /*String*/
      $scope.defaultTitle = 'Desayuno 0,99\u20ac';
      // espejo del titulo con un valor predeterminado
      /*String*/
      $scope.defaultDescription = 'Esto es un desayuno que incluye caf\xe9, zumo y fruta.';
      //espejo de la descripcion con valor predeterminado.
      /*String*/
    ...
    $scope.title = 'Tarjeta de Informaci\xf3n';
    /*String*/
    $scope.plctitleInfocard = 'Desayuno 0,99\u20ac';
    /*String*/
    $scope.plcdescription = 'Esto es un desayuno que incluye caf\xe9, zumo y fruta.';
    /*String*/
    $scope.urlImgOrgBackground = '';
    /*List*/
    $scope.visibility = {};
    ...
    $scope.images = {};
    asm.getEntityOfKindById(Organization.entityKind, asm.getCurrentOrganizationId()).then(function (entity) {
      if (entity != null) {
        org = new Organization();
        org.initFromEntity(entity);
        $scope.urlImgOrgBackground = org.urlImgBackground;
    ...
    $scope.$watch('tmpInfocard.description', function (newValue, oldValue) {
      newValue.length == 0 ? $scope.defaultDescription = 'Esto es un desayuno que incluye caf\xe9, zumo y fruta.' : $scope.defaultDescription = newValue;
    ...
    $scope.guardarCambios = function () {
      ...
      $scope.boton = true;
      if ($scope.bmImgBack)
        fileImg.push($scope.images.fileUrlImg);
      if (fileImg.length > 0) {
        asm.uploadImages(fileImg).then(function (res) {
          if ($scope.bmImgBack)
            $scope.tmpInfocard.urlImg = res[$scope.images.fileUrlImg.name];
...
fibity.manager.app.controller('GTCampaignsVC', [
  '$scope',
  '$http',
  '$routeParams',
  '$modal',
  function ($scope, $http, $routeParams, $modal) {
    ...
    $scope.path = [{
        'name': 'Campa\xf1as',
        'href': '#/campaigns/collection'
      }];
    $scope.title = 'Campa\xf1as';
    $scope.activeitem = 'campaigns';
    $scope.antennasSchedulesMap = {};
    //$scope.param = $routeParams.param;
    /*List<String>*/
    $scope.cabeceras = [
      'Nombre de la Campa\xf1a',
      'InfoCard',
      'Antenas',
      'Editar'
    ];
    ...
    /*bool*/
    $scope.btnCamp2;
    /*Map<String, CampaignsSchedule>*/
    $scope.schedulecampList = {};
    /*List<String>*/
    $scope.schedulecampKeyList = [];
    /*Schedule*/
    $scope.tmpCampaign;
    /*Map<String>*/
    $scope.infoCardList = [];
    //Mapa que contiene todos los objetos Infocard del modelo
    /*Map<String>*/
    $scope.infoCardMap = {};
    //Mapa que contiene todos los objetos Infocard del modelo
    /*List<String>*/
    ...
    $scope.updateAntennasScheduleMap = function () {
      ...
      Campaign.fromEntityList(asm.getAllEntitiesOfKind(Campaign.entityKind)).then(function (list) {
          ...
          if (camp.getAntennasList() != undefined) {
            camp.getAntennasList().forEach(function (antennaId) {
              if (camp.getSchedulesList() != undefined) {
                $scope.antennasSchedulesMap[antennaId].push({
                  'campaignId': camp.id,
                  'scheduleId': camp.getSchedulesList()[0]
                });
              } else {
    ...
    $scope.openCampaignsScheduleModal = function (id) {
      ...
      modalInstance.result.then(function (tmpCampaign) {
        tmpCampaign.save().then(function (mapa) {
...
fibity.manager.app.controller('GTCampaignAddModalVC', [
  '$scope',
  '$modalInstance',
  '$timeout',
  'tmpCampaign',
  'antennasSchedulesMap',
  function ($scope, $modalInstance, $timeout, tmpCampaign, antennasSchedulesMap) {
    console.log(antennasSchedulesMap);
    /*Campaign*/
    $scope.tmpCampaign = new Campaign();
    //Copia de la campañaña
    /*List<String>*/
    $scope.campaignsKeyList = [];
    //Lista que contiene todos los ids de Campaña del modelo
    $scope.campaignsMap = {};
    //Mapa que contiene todos los objetos Campaña del modelo 
    /*List<String>*/
    $scope.infoCardsKeyList = [];
    //Lista que contiene todos los ids de Infocard del modelo
    /*Map<String>*/
    $scope.infoCardsMap = {};
    //Mapa que contiene todos los objetos Infocard del modelo
    /*Map<String>*/
    $scope.AntennasMap = {};
    //Mapa que contiene todos las antenas del modelo
    $scope.AntennasKeyList = [];
    $scope.AntennasKeyListValid = [];
    // Antenas filtradas que cumplen las condiciones de los modos estatico y dinamico.
    $scope.AntennasKeyListSelected = [];
    // Antenas asociadas a la campaña
    $scope.AntennasKeyListAvailable = [];
    // = AntennasKeyListValids - AntennasKeyListSelected
    /*Map<String, Schedule Calendar>*/
    $scope.scheduleList = {};
    /*List<String>*/
    $scope.scheduleKeyList = [];
    /*Schedule*/
    $scope.tmpschedule;
    /*Map<ScheduleId,List<AntennaId>>*/
	 ...
	 */
    $scope.msgs = {};
    $scope.msgs.enableDynamicMode = 'Activar modo din\xe1mico';
    $scope.msgs.disableDynamicMode = 'Desactivar modo din\xe1mico';
    $scope.msgs.dynamicMode = 'Din\xe1mica';
    $scope.msgs.staticMode = 'Est\xe1tica';
    /**
	...
	*/
    ...
    $scope.txtDinamic = 'Din\xe1mico';
    $scope.txtStatic = 'Est\xe1tico';
    $scope.btnStatic = 'Activar modo est\xe1tico';
    $scope.btnDinamic = 'Desactivar modo est\xe1tico';
    /*bool*/
    $scope.viewAnt = true;
    $scope.show = false;
    $scope.phase2 = false;
    ...
    $scope.loadSchedules = function () {
      Schedule.fromEntityList(asm.getAllEntitiesOfKind(Schedule.entityKind)).then(function (list) {
    ...
    $scope.updateCalendarForCampaignAntenas = function () {
      ...
      $scope.mapSchedulesExt = {};
      $scope.listSchedulesExt = [];
      $scope.AntennasKeyListSelected.forEach(function (antId) {
    ...
    $scope.addAntenaInMapSchedulesExt = function (antId) {
      console.log('addAntenaInMapSchedulesExt()');
      sid = '';
      if (tmpCampaign.getSchedulesList() != undefined) {
        sid = tmpCampaign.getSchedulesList()[0];
      }
      if (antennasSchedulesMap[antId] != undefined) {
        antennasSchedulesMap[antId].forEach(function (data) {
    ...
    $scope.staticValidFilter = function () {
      ...
      for (key in antennasSchedulesMap) {
        ...
        if (valid)
          $scope.AntennasKeyListValid.push(key);
      }
    ...
    $scope.dynamicValidFilter = function () {
      ...
      for (key in antennasSchedulesMap) {
        //value = ARRAY[{"campaignId" :id "scheduleId": [id,...]}]
        var valid = true;
        //Supenemos que la antena cumple las condiciones, debemos comprobar si realmente las cumple.
        var value = antennasSchedulesMap[key];
          ...
          if (antennaAsociateInfo.campaignId != $scope.tmpCampaign.id) {
            ...
            if (!camp.dynamicMode) {
              ...
              if (antennaAsociateInfo.scheduleId != undefined && comprobarEventos) {
                ...
                var eventValidator = new EventValidator(schedules, {
                    start: schedule.dateIni,
                    end: schedule.dateEnd,
                    allDay: schedule.allDay
                  });
    ...
    $scope.calculateAntennasKeyListAvailable = function () {
      ...
      for (key in mapa) {
        if (mapa[key])
          $scope.AntennasKeyListAvailable.push(key);
      }
    };
    ...
    $scope.loadDataView = function () {
      if (tmpCampaign.id != '') {
        /*String*/
        $scope.tmpCampaign.name = tmpCampaign.name;
        /*String*/
        $scope.optionsInfocard.optInfo = $scope.infoCardsMap[tmpCampaign.infocardId];
        /*String*/
        $scope.urlImg = $scope.infoCardsMap[tmpCampaign.infocardId].urlImg;
        /*String*/
        $scope.txtTitle = $scope.infoCardsMap[tmpCampaign.infocardId].title;
        /*String*/
        $scope.txtDescription = $scope.infoCardsMap[tmpCampaign.infocardId].description;
        var json = JSON.stringify(tmpCampaign.antennasList);
        $scope.AntennasKeyListSelected = JSON.parse(json);
        /*bool*/
        $scope.tmpCampaign.dynamicMode = tmpCampaign.dynamicMode;
        /*bool*/
        $scope.oculto = true;
        /*bool*/
        $scope.boton = false;
      ...
      if (tmpCampaign.getSchedulesList() != undefined) {
        // La campaña ya tiene asignada un Schedule
        sid = tmpCampaign.getSchedulesList()[0];
        asm.getEntityOfKindById(Schedule.entityKind, sid).then(function (entity) {
          if (entity != null) {
        ...
        // La campaña no tiene asignada un Schedule
      ...
      $scope.$watch('optionsAntennas.optAnt', function (selectedObject) {
        ...
        if (selectedObject != '' && selectedObject != undefined && selectedObject != null) {
          ...
          if (j != -1) {
            $scope.AntennasKeyListAvailable.splice(j, 1);
          }
          $scope.AntennasKeyListAvailable.length == 0 ? $scope.viewAnt = false : $scope.viewAnt = true;
          $scope.updateCalendarForCampaignAntenas();
        }
      ...
      $scope.deleteAnt = function (Ant) {
        ...
        if (i != -1) {
          $scope.AntennasKeyListSelected.splice(i, 1);  //Asignar al calendario
        }
        $scope.updateCalendarForCampaignAntenas();
        $scope.optionsAntennas.optAnt = '';
      };
      ...
      $scope.addEvent = function (start, end, allDay, jsEvent, view) {
        ...
        if (!$scope.editingEvent) {
          ...
          if (start != undefined && end != undefined && $scope.tmpCampaign.name != undefined) {
            var eventValidator = new EventValidator($scope.eventSources[$scope.calEventsExt], {
                start: start,
                end: end,
                allday: $scope.datosEvents.allDay
              });
            if ($scope.eventSources[$scope.calEvents].length == 0) {
              console.log('no hab\xeda evento, pero ahora si');
              $scope.eventSources[$scope.calEvents][$scope.actualEvent] = {};
            }
            if ($scope.datosEvents.allDay) {
              if (eventValidator.isValid()) {
                ...
                if ($scope.tmpCampaign.title == '') {
                  $scope.eventSources[$scope.calEvents][n].title = 'Campa\xf1a';
                } else {
        ...
        if ($scope.tmpCampaign.dynamicMode) {
          ...
          $scope.tmpschedule.save().then(function (mapa) {
            ...
            for (sid in tmpCampaign.getSchedulesList()) {
              if (sid == $scope.tmpschedule.id)
                encontrado = true;
            }
            if (!encontrado)
              tmpCampaign.addScheduleToList($scope.tmpschedule.id);
          }).finally(function () {
    ...
    $scope.load = function () {
      ...
      $scope.chartObject = {
          ...
          'rows': [
            {
              'c': [
                ...
                {
                  'v': 340,
                  'f': '340 Usuarios detectaron esta informaci\xf3n'
                },
                {
                  'v': 124,
                  'f': '124 usuarios se interesaron por la informaci\xf3n'
                }
              ]
            },
            {
              'c': [
                ...
                {
                  'v': 105,
                  'f': '105 Usuarios detectaron esta informaci\xf3n'
                },
                {
                  'v': 48,
                  'f': '48 usuarios se interesaron por la informaci\xf3n'
                }
              ]
            },
            {
              'c': [
                ...
                {
                  'v': 89,
                  'f': '89 Usuarios detectaron esta informaci\xf3n'
                },
                {
                  'v': 36,
                  'f': '36 usuarios se interesaron por la informaci\xf3n'
                }
              ]
            },
            {
              'c': [
                ...
                {
                  'v': 185,
                  'f': '185 Usuarios detectaron esta informaci\xf3n'
                },
                {
                  'v': 65,
                  'f': '65 usuarios se interesaron por la informaci\xf3n'
                }
              ]
            },
            {
              'c': [
                ...
                {
                  'v': 250,
                  'f': '250 Usuarios detectaron esta informaci\xf3n'
                },
                {
                  'v': 160,
                  'f': '160 usuarios se interesaron por la informaci\xf3n'
                }
              ]
            },
            {
              'c': [
                ...
                {
                  'v': 545,
                  'f': '545 Usuarios detectaron esta informaci\xf3n'
                },
                {
                  'v': 289,
                  'f': '289 usuarios se interesaron por la informaci\xf3n'
                }
              ]
            },
            {
              'c': [
                ...
                {
                  'v': 369,
                  'f': '369 Usuarios detectaron esta informaci\xf3n'
                },
                {
                  'v': 198,
                  'f': '198 usuarios se interesaron por la informaci\xf3n'
                }
              ]
            }
          ]
        ...
        'options': {
          ...
          'vAxis': {
            'title': 'N\xfamero de usuarios',
            'gridlines': { 'count': 10 }
          },
          'hAxis': { 'title': 'D\xeda de la semana' },
          'lineWidth': 3,
          'colors': [
            '#1c91c0',
            '#5bb85b'
...
fibity.manager.app.controller('GTCustomersVC', [
  '$scope',
  '$routeParams',
  function ($scope, $routeParams) {
    /******** Profile for TopBar Component *******/
    ...
    /*String*/
    $scope.titleList = 'Vista Modo Lista';
    /*String*/
    $scope.titleCollec = 'Vista Modo Icono';
    /*Map*/
    $scope.customersList = {};
    //Mapa que contiene todos los objetos antenna del modelo
    /*List<String>*/
    $scope.customersKeyList = [];
    // Lista de ids de los objetos antenna del modelo
    Customers.fromEntityList(asm.getAllEntitiesOfKind(Customers.entityKind)).then(function (list) {
