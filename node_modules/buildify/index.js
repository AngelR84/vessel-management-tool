...
/**
 * Perform a annotate to manipulate or use the content, function must return the content
 *
 ...
 */
Builder.prototype.htmlmin = function() {
  ...
  var output = minify(this.content,options);
 ...
 * @param {Boolean} [options.mangle]    Whether to mangle variable names etc. Default: true
 */
Builder.prototype.annotate = function(options) {
  const renameOptions = [
	                       {from: "$super", to: "$super"}
	                   ];
  var output = ngAnnotate(this.content,{add: true, rename: renameOptions}).src;
...
Builder.prototype.uglify = function(options) {
  ...
  output = uglify.ast_mangle(output, options);
